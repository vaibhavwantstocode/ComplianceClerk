...
from config import OPENAI_API_KEY_ENV, OPENAI_BASE_URL, OPENAI_VISION_MODEL
...
def _openai_generate(prompt: str, image_bytes: bytes) -> str:
    api_key = __import__("os").getenv(OPENAI_API_KEY_ENV, "").strip()
    if not api_key:
    ...
    try:
        ...
        # Compatibility fallback for providers that reject response_format.
        if exc.code in (400, 422):
            ...
            try:
                ...
                raise RuntimeError(f"OpenAI HTTP {exc2.code}: {details2}") from exc2
...
def run_openai_with_retry(
    primary_prompt: str,
    image_bytes: bytes,
    required_fields: list[str],
    retry_context: str,
) -> tuple[dict, str, str, list[str]]:
    primary_raw = _openai_generate(primary_prompt, image_bytes)
    ...
    retry_raw = _openai_generate(retry_prompt, image_bytes)
